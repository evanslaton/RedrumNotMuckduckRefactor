...
namespace redrum_not_muckduck_game
{
    class SaveElements
    {
        ...
        public static void GetWorkingElementDirectory()
        {
            if (Game.Is_Windows)
            {
                WorkingElementDirectory = Environment.CurrentDirectory.Replace("bin\\Debug\\netcoreapp3.1\\save\\save-data", "Elements.json");
            }
            else
            {
                WorkingElementDirectory = Environment.CurrentDirectory.Replace("bin/Debug/netcoreapp3.1/save/save-data", "Elements.json");
            }
        }
    }
}
